    ...
    <div class="moduleElHight">
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                ref="multipleTable" @selection-change="handleSelectionChange" @sort-change="shortChange" @sort-change="shortChange" :default-sort="{ prop: 'id', order: 'descending' }" v-loading="loading">
                <template slot="empty">
                    <p>{{dataText}}</p>
                </template>
                ...
                <el-table-column prop="datetime" sortable="custom" label="浏览时间" width="140">
...
module.exports = {
    ...
    methods: {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=company_comlog&a=lookjob', params,{hideloading: true}).then(function (response) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=company_comlog&a=dellookjob', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
