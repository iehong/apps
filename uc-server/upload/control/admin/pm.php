...
class control extends adminbase {
	...
	public function onls() {
		...
		if($this->submitcheck() || getgpc('searchpmsubmit', 'G')) {
			...
			if(!empty($wheresql)) {
				$count = $this->db->result_first('SELECT COUNT(*) FROM '.UC_DBTABLEPRE.'pm_messages_'.$srchtablename.' WHERE '.implode(' AND ', $wheresql));
			}
			if($count) {
				...
				$limit = UC_PPP;
				$user = [];
				$query = $this->db->query('SELECT * FROM '.UC_DBTABLEPRE.'pm_messages_'.$srchtablename.' WHERE '.implode(' AND ', $wheresql)." LIMIT $start, $limit");
				...
				foreach($pmlist as $key => $value) {
					$pmlist[$key]['author'] = $usernamearr[$value['authorid']];
				}
		...
		$pmnum = 0;
		for($i = 0; $i < 10; $i++) {
			$pmnum += $this->db->result_first('SELECT COUNT(*) FROM '.UC_DBTABLEPRE.'pm_messages_'.$i);
		}
	...
	public function onclear() {
		...
		if($this->submitcheck() || getgpc('clearpmsubmit', 'G')) {
			...
			if($usernames) {
				...
				if($processed) {
					$this->message('pm_clear_processing', $nexturl, 0, ['current' => $current, 'next' => $next]);
				} else {
