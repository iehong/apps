 ...
 */
class category_px_subject_class_controller extends adminCommon
{
    /**
     * 培训分类 课程类别
     */
    function index_action()
    {
        ...
        $position = $categoryM->getSubjectClassList($whereData);
    ...
    function save_action()
    {
        ...
        if ($_POST) {
            ...
            $categoryM = $this->MODEL('category');
            $return = $categoryM->addSubjectClass($addData);
    ...
    function up_action()
    {
        ...
        if ($_POST['id']) {
            ...
            $class1 = $categoryM->getSubjectClass($whereOne);
            $class2 = $categoryM->getSubjectClassList($whereTwo);

            $return['class1'] = $class1;
            $return['class2'] = $class2;
        }
        $position = $categoryM->getSubjectClassList(array('keyid' => '0'));
    ...
    function del_action()
    {
        ...
        if ($whereData) {
            $return = $categoryM->delSubjectClass($whereData, $data);
            $this->admin_json($return['error'], $return['msg']);
        }
    }
    ...
    function ajax_action()
    {
        if ($_POST) {
            ...
            $return = $categoryM->addSubjectClass($addData, $whereData);
            $this->admin_json($return['error'], $return['msg']);
        }
    }
}