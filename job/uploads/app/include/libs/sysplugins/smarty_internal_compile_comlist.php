...
class Smarty_Internal_Compile_Comlist extends Smarty_Internal_CompileBase{
    ...
    public function compile($args, $compiler, $parameter){
		...
		//关键字
		if($paramer[\'keyword\']){
			$where.=" AND (`name` LIKE \'%".$paramer[\'keyword\']."%\' OR `shortname` LIKE \'%".$paramer[\'keyword\']."%\')";
		}				
		//公司行业
		if($paramer[\'hy\']){
		...
		// 数据周期 
		if($config[sy_datacycle_com]>0){
		
            $uptime =   strtotime(\'-\'.$config[sy_datacycle_com].\' day\');
            $where.=" AND `lastupdate`>\'".$uptime."\'";
	    }
		if($paramer[\'jobtime\']){
			$where.=" AND `jobtime`<>\'\'";
		...
		if($paramer[\'isjob\']){
			...
			if($JobId!=""){
			    if($config[sy_datacycle_job]>0){
			        ...
			        $JobList=$db->select_all("company_job","`uid` IN ($JobId) and r_status=1 and status=0 and is_depower=2 and state=1 and lastupdate > $uptime  order by `lastupdate` desc","`id`,`uid`,`status`,`name`");	
			    }else{
			    
				    $JobList=$db->select_all("company_job","`uid` IN ($JobId) and r_status=1 and status=0 and is_depower=2 and state=1  order by `lastupdate` desc","`id`,`uid`,`status`,`name`");
				}
			}
