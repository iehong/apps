...
const PRIVATEPMTHREADLIMIT_ERROR = -1;
const PMFLOODCTRL_ERROR = -2;
const PMMSGTONOTFRIEND = -3;
const PMSENDREGDAYS = -4;
const CHATPMTHREADLIMIT_ERROR = -5;
const CHATPMMEMBERLIMIT_ERROR = -7;
...
class pmcontrol extends base {
	...
	public function oncheck_newpm() {
		...
		if($more == 0) {
			...
			return ['newpm' => $newpm, 'newprivatepm' => $newprvpm];
		} elseif($more == 2 || $more == 3) {
			if($more == 2) {
				return ['newpm' => $newpm, 'newprivatepm' => $newprvpm, 'newchatpm' => $newchatpm];
			} else {
				...
				return ['newpm' => $newpm, 'newprivatepm' => $newprvpm, 'newchatpm' => $newchatpm, 'lastdate' => $lastpm['lastdateline'], 'lastmsgfromid' => $lastpm['lastauthorid'], 'lastmsgfrom' => $lastpm['lastauthorusername'], 'lastmsg' => $lastpm['lastsummary']];
			}
		} elseif($more == 4) {
			return ['newpm' => $newpm, 'newprivatepm' => $newprvpm, 'newchatpm' => $newchatpm];
		} else {
	...
	public function ondeleteuser() {
		$this->init_input();
		$this->user['uid'] = intval($this->input('uid'));
		return $_ENV['pm']->deletepmbyplids($this->user['uid'], $this->input('touids'), 1);
	}
	...
	public function onls() {
		...
		$filter = $filter ? (in_array($filter, ['newpm', 'privatepm']) ? $filter : '') : '';
		if($filter == 'newpm') {
			$type = 0;
			$new = 1;
		...
		if($pagesize > 0) {
			...
			if(is_array($pms) && !empty($pms)) {
				foreach($pms as $key => $pm) {
					if($msglen) {
						$pms[$key]['lastsummary'] = $_ENV['pm']->removecode($pm['lastsummary'], $msglen);
					} else {
	...
	public function onview() {
		...
		if($pms) {
			foreach($pms as $key => $pm) {
				$pms[$key]['message'] = $this->uccode->complie($pm['message']);
			}
		}
		return $pms;
	}
	...
	public function onviewnum() {
		...
		return $_ENV['pm']->getpmnumbyplid($this->user['uid'], $plid);
	}
