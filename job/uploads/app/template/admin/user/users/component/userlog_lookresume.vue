    ...
    <div class="moduleElHight">
                ...
                <div class="moduleInptList" style="margin-bottom: 8px;">
                    <el-input placeholder="输入你要搜索的关键字" @keyup.enter.native="handleSearch" size="small" v-model="searchForm.keyword" class="input-with-select" clearable>
                        <el-select v-model="searchForm.type" slot="prepend" placeholder="请选择">
                            <el-option label="人才姓名" value="1"></el-option>
                            <el-option label="被浏览简历" value="2"></el-option>
                            <el-option label="企业名称" value="3"></el-option>
                        </el-select>
                    </el-input>
                </div>
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="浏览开始日期" end-placeholder="浏览结束日期" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column prop="username" label="人才姓名 ">
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.username }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="resume_name" label="被浏览简历" min-width="150" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.resume_name }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="datetime" label="浏览时间" width="150" sortable="custom">
...
module.exports = {
    ...
    methods: {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=users_userlog&a=lookresume', params,{hideloading: true}).then(function (response) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=users_userlog&a=dellook', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
