    ...
    <div class="moduleElHight">
        <div class="moduleElSearchInf">
            <div class="moduleElTabInpt" style="flex-wrap: wrap;">
                ...
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="下载开始日期" end-placeholder="下载结束日期" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column label="企业/联系人/手机号" width="230">
                    <template slot-scope="scope">
                        <div class=" ">
                            <div class="username">
                                <el-link :href="scope.row.com_url" target="_blank" type="primary">{{ scope.row.com_name }}</el-link>
                            </div>
                            <div class=" ">{{ scope.row.com_username }} {{ scope.row.telPho }}</div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="人才/手机号">
                    <template slot-scope="scope">
                        <div class=" ">
                            <div class="username">
...
module.exports = {
    ...
    methods: {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=users_userlog&a=freedown', params,{hideloading: true}).then(function (response) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=users_userlog&a=delfreedown', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
