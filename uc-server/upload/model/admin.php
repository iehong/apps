...
class adminbase extends base {
	...
	public function is_founder($username) {
		return $this->user['isfounder'];
	}

	public function writelog($action, $extra = '') {
		...
		file_put_contents($logfile, "<?PHP exit;?>\t".str_replace(['<?', '?>', '<?php'], '', $log)."\n", FILE_APPEND);
	}
	...
	public function sid_encode($username) {
		$ip = $this->onlineip;
	...
	public function sid_decode($sid) {
		$ip = $this->onlineip;
		$agent = $_SERVER['HTTP_USER_AGENT'];
		$authkey = md5($ip.$agent.UC_KEY);
