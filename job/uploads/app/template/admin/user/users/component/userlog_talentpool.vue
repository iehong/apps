    ...
    <div class="moduleElHight">
                ...
                <div class="moduleInptList">
                    <el-input placeholder="输入你要搜索的关键字" @keyup.enter.native="handleSearch" size="small" v-model="searchForm.keyword" class="input-with-select" clearable>
                        <el-select v-model="searchForm.type" slot="prepend" placeholder="请选择">
                            <el-option label="被收藏人才" value="1"></el-option>
                            <el-option label="公司名称" value="2"></el-option>
                        </el-select>
                    </el-input>
                </div>
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="收藏开始日期" end-placeholder="收藏结束日期" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column prop="com_name" label="公司名称" min-width="150" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-link :href="scope.row.com_url" target="_blank" type="primary">{{ scope.row.com_name }}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="username" label="被收藏人才" width="150">
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.username }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="jobname" label="求职意向" min-width="180"></el-table-column>
                <el-table-column prop="exp" label="工作经验" width="150"></el-table-column>
                <el-table-column prop="salary" label="期望薪资" width="150"></el-table-column>
                <el-table-column prop="ctime_n_n" label="收藏时间" width="160"></el-table-column>
...
module.exports = {
    ...
    methods: {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=users_userlog&a=talentpool', params,{hideloading: true}).then(function (response) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=users_userlog&a=deltalentpool', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
