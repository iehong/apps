...
class usermodel {
	...
	public function get_user_by_uid($uid) {
		return $this->db->fetch_first('SELECT * FROM '.UC_DBTABLEPRE."members WHERE uid='$uid'");
	}

	public function get_user_by_username($username) {
		return $this->db->fetch_first('SELECT * FROM '.UC_DBTABLEPRE."members WHERE username='$username'");
	}

	public function get_user_by_email($email) {
		return $this->db->fetch_first('SELECT * FROM '.UC_DBTABLEPRE."members WHERE email='$email'");
	}

	public function get_user_by_secmobile($secmobicc, $secmobile) {
	...
	public function check_username($username) {
		...
		$len_max = $this->base->settings['username_len_max'] ?? 15;
		if($len > $len_max || $len < 3 || preg_match("/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&\(\)']|$guestexp/is", $username)) {
			return FALSE;
		} else {
			return TRUE;
		}
	...
	public function check_mergeuser($username) {
		return $this->db->result_first('SELECT count(*) FROM '.UC_DBTABLEPRE."mergemembers WHERE appid='".$this->base->app['appid']."' AND username='$username'");
	}
	...
	public function check_usernameexists($username) {
		return $this->db->result_first('SELECT username FROM '.UC_DBTABLEPRE."members WHERE username='$username'");
	}
	...
	public function check_emailexists($email, $username = '') {
		...
		return $this->db->result_first('SELECT email FROM  '.UC_DBTABLEPRE."members WHERE email='$email' $sqladd");
	}

	public function check_secmobileexists($secmobicc, $secmobile, $username = '') {
		$sqladd = $username !== '' ? "AND username<>'$username'" : '';
		$secmobicc == 0 && $secmobicc = '';
		$secmobile == 0 && $secmobile = '';
		return $this->db->result_first('SELECT secmobile FROM  '.UC_DBTABLEPRE."members WHERE secmobicc='$secmobicc' AND secmobile='$secmobile' $sqladd");
	}
	...
	public function add_user($username, $password, $email = '', $uid = 0, $questionid = '', $answer = '', $regip = '', $secmobicc = '', $secmobile = '') {
	...
	public function edit_user($username, $oldpw, $newpw, $email = '', $ignoreoldpw = 0, $questionid = '', $answer = '', $secmobicc = '', $secmobile = '') {
		...
		$sqladd = $newpw ? "password='".$this->generate_password($newpw)."', salt='{$_ENV['password_salt']}'" : '';
		$sqladd .= ($sqladd ? ',' : '')." email='$email'";
	...
	public function set_user_field($username, $fields) {
		$sqladd = '';
	...
	public function chgusername($uid, $newusername) {
	...
	public function get_total_num($sqladd = '') {
		return $this->db->result_first('SELECT COUNT(*) FROM '.UC_DBTABLEPRE."members $sqladd");
	}

	public function get_list($page, $ppp, $totalnum, $sqladd) {
		$start = $this->base->page_get_start($page, $ppp, $totalnum);
		return $this->db->fetch_all('SELECT * FROM '.UC_DBTABLEPRE."members $sqladd LIMIT $start, $ppp");
	}
	...
	public function can_do_login($username, $ip = '') {
		...
		if($ip_check || $user_check) {
			return min(($check_times - (isset($ip_check['count']) ? $ip_check['count'] : 0)), ($check_times - (isset($user_check['count']) ? $user_check['count'] : 0)));
	...
	public function user_log($uid, $action, $extra = '') {
		$uid = intval($uid);
	...
	public function get_passwordoptions() {
	...
	public function generate_password($password) {
		$_ENV['password_salt'] = '';
		if(defined('UC_PASSWORD_WITH_SALT')) {
			$_ENV['password_salt'] = $salt = substr(uniqid(rand()), -6);
	...
	public function upgrade_password($username, $password, $hash, $salt = '') {
		$algo = $this->get_passwordalgo();
		$options = $this->get_passwordoptions();
	...
	public function reset_founderpw($newpw, $reconfkey = 1) {
	...
	public function upgrade_founderpw($password, $hash, $salt = '') {
		$algo = $this->get_passwordalgo();
		$options = $this->get_passwordoptions();
		if(!empty($salt) || password_needs_rehash($hash, $algo, $options)) {
			$password_new = $this->generate_password($password);
	...
	public function alter($sql) {
		...
		$return = $this->db->query('ALTER TABLE '.UC_DBTABLEPRE.'members '.$sql);
