...
class pm_clientcontrol extends base {
	...
	public function pm_clientcontrol() {
		parent::__construct();
	...
	public function onls() {
		...
		$page = $page ? $page : 1;
		$filter = $filter ? (in_array($filter, ['newpm', 'privatepm']) ? $filter : '') : 'privatepm';
	...
	public function onsend() {
		...
		$friends = $totalnum ? $_ENV['friend']->get_list($this->user['uid'], 1, $totalnum, $totalnum, 3) : [];
		if(!$this->submitcheck()) {
			...
			if($this->settings['pmsendregdays']) {
				if($user['regdate'] > $this->time - $this->settings['pmsendregdays'] * 86400) {
					$this->message('pm_send_regdays_error', 'BACK', 1, ['$pmsendregdays' => $this->settings['pmsendregdays']]);
				}
			}
			if($this->settings['chatpmmemberlimit']) {
				if($type == 1 && ($countmsgto > ($this->settings['chatpmmemberlimit'] - 1))) {
					$this->message('pm_send_chatpmmemberlimit_error', 'BACK', 1, ['$chatpmmemberlimit' => $this->settings['chatpmmemberlimit']]);
				}
			}
			if($this->settings['pmfloodctrl']) {
				if(!$_ENV['pm']->ispminterval($this->user['uid'], $this->settings['pmfloodctrl'])) {
					$this->message('pm_send_pmfloodctrl_error', 'BACK', 1, ['$pmfloodctrl' => $this->settings['pmfloodctrl']]);
				}
			}
			if($this->settings['privatepmthreadlimit']) {
				if(!$_ENV['pm']->isprivatepmthreadlimit($this->user['uid'], $this->settings['privatepmthreadlimit'])) {
					$this->message('pm_send_privatepmthreadlimit_error', 'BACK', 1, ['$privatepmthreadlimit' => $this->settings['privatepmthreadlimit']]);
				}
			}
			if($this->settings['chatpmthreadlimit']) {
				if(!$_ENV['pm']->ischatpmthreadlimit($this->user['uid'], $this->settings['chatpmthreadlimit'])) {
					$this->message('pm_send_chatpmthreadlimit_error', 'BACK', 1, ['$chatpmthreadlimit' => $this->settings['chatpmthreadlimit']]);
				}
			}
