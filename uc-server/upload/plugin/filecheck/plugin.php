...
class control extends pluginbase {
	...
	public function onindex() {
		...
		foreach($showlist as $file => $md5) {
			...
			if(file_exists($file)) {
				$filemtime = @filemtime($file);
				$fileststus && $dirlist[$fileststus][$dir][basename($file)] = [number_format(filesize($file)).' Bytes', $this->date($filemtime)];
			} else {
