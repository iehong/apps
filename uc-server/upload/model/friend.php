...
class friendmodel {
	...
	public function get_totalnum_by_uid($uid, $direction = 0) {
		...
		return $this->db->result_first('SELECT COUNT(*) FROM '.UC_DBTABLEPRE."friends WHERE $sqladd");
	}
	...
	public function get_list($uid, $page, $pagesize, $totalnum, $direction = 0) {
		...
		if($sqladd) {
			return $this->db->fetch_all('SELECT f.*, m.username FROM '.UC_DBTABLEPRE.'friends f LEFT JOIN '.UC_DBTABLEPRE."members m ON f.friendid=m.uid WHERE $sqladd LIMIT $start, $pagesize");
		} else {
			return [];
		}
	}
