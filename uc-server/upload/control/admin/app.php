...
class control extends adminbase {
	...
	public function onadd() {
		if(!$this->submitcheck()) {
			...
			$a = getgpc('a');
			$this->view->assign('a', $a);
			$typelist = ['DISCUZX' => 'DiscuzX', 'UCHOME' => 'UCenter Home', 'XSPACE' => 'X-Space', 'DISCUZ' => 'Discuz!', 'SUPESITE' => 'SupeSite', 'SUPEV' => 'SupeV', 'ECSHOP' => 'ECShop', 'ECMALL' => 'ECMall', 'OTHER' => $this->lang['other']];
			...
			if($app) {
				...
				$extra = serialize(['apppath' => getgpc('apppath', 'P')]);
	...
	public function ondetail() {
			...
			if(getgpc('apppath', 'P')) {
				...
				if(!empty($app['extra']['apppath'])) {
					...
					if(!file_exists($apifile)) {
						$this->message('app_apifile_not_exists', 'BACK', 0, ['$apifile' => $apifile]);
					}
					...
					if(!$uc_client_version || $uc_client_version <= '1.0.0') {
						$this->message('app_apifile_too_low', 'BACK', 0, ['$apifile' => $apifile]);
					}
				} else {
		...
		$tagtemplates = is_array($tagtemplates) ? $tagtemplates : [];
		...
		$synloginchecked = [$app['synlogin'] => 'checked="checked"'];
		$recvnotechecked = [$app['recvnote'] => 'checked="checked"'];
	...
	private function _add_note_for_app() {
		$this->load('note');
	...
	private function _format_notedata($notedata) {
		$arr = [];
