...
class seccode {
	...
	public $code;                        //100000-999999 范围内随机
	public $type = 0;                //0 英文图片验证码  1 中文图片验证码  2 Flash 验证码  3 语音验证码
	public $width = 0;
	public $height = 0;
	public $background = 1;
	public $adulterate = 1;
	...
	public static function seccodeconvert(&$seccode) {
	...
	public function adulteratefont() {
	...
	public function ttffont() {
		...
		while($entry = readdir($dirs)) {
			if($entry != '.' && $entry != '..' && in_array(strtolower($this->fileext($entry)), ['ttf', 'ttc'])) {
				$seccodettf[] = $entry;
			}
		}
		if(empty($seccodettf)) {
	...
	public function swfcode($width, $d, $code, $order) {
		$str = '';
