...
class control extends adminbase {
	...
	public function onls() {
		...
		$operate = getgpc('o');
		if($operate == 'list') {
			if($delete = (isset($_POST['delete']) ? $_POST['delete'] : [])) {
				if(is_array($delete)) {
					foreach($delete as $filename) {
						@unlink('./data/backup/'.str_replace(['/', '\\'], '', $filename));
					}
				}
				$status = 2;
			...
			if(is_dir(UC_ROOT.'./data/backup/')) {
				...
				while($entry = $dir->read()) {
					...
					if(is_dir($file) && preg_match('/backup_(\d+)_\w+/i', $file, $match)) {
						$baklist[] = ['name' => $match[0], 'date' => $match[1]];
					}
				}
				$dir->close();
			} else {
	...
	public function onoperate() {
		...
		if(empty($arr['fileinfo'])) {
			...
			$this->message($this->_parent_js($appid, 'db_'.$type.'_multivol_redirect', ['$volume' => $arr['fileinfo']['file_num']]), UC_ADMINSCRIPT.'?m=db&a=operate&t='.$type.'&appid='.$appid.'&nexturl='.urlencode($arr['nexturl']));
	...
	public function ondelete() {
		...
		if($next_appid != $appid) {
			$this->message($this->_parent_js($backupdir, 'delete_dumpfile_redirect', ['$appname' => $appname]), UC_ADMINSCRIPT.'?m=db&a=delete&appid='.$next_appid.'&backupdir='.$backupdir.'&sid='.$this->sid);
		} else {
			$this->message($this->_parent_js($backupdir, 'delete_dumpfile_success'));
		}
	}

	private function _next_appid($appid) {
		$last_appid = 0;
	...
	private function _parent_js($extid, $message, $vars = []) {
	...
	private function _xml2array($xml) {
		...
		$arr['error'] = ['errorcode' => $match[1], 'errormessage' => $match[2]];
		return $arr;
	}

	public function sqldumptable($table, $startfrom = 0, $currsize = 0) {
		$offset = 300;
		$tabledump = '';
		$usehex = TRUE;
		$tablefields = [];
