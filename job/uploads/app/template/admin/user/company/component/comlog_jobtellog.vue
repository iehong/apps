                ...
                <div class="moduleInptList">
                    <el-input placeholder="输入你要搜索的关键字" @keyup.enter.native="handleSearch" size="small" v-model="searchForm.keyword" class="input-with-select" clearable>
                        <el-select v-model="searchForm.type" slot="prepend" placeholder="请选择">
                            <el-option label="拨号人" value="1"></el-option>
                            <el-option label="职位名称" value="2"></el-option>
                            <el-option label="所属企业" value="3"></el-option>
                            <el-option label="拨号人ip" value="4"></el-option>
                        </el-select>
                    </el-input>
                </div>
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="拨号开始日期" end-placeholder="拨号结束日期" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
                <div v-for="(searchItem, searchIndex) in searchList" :key="searchIndex" class="moduleInptList">
                    <el-select size="small" v-model="searchForm[searchItem.param]" slot="prepend" :clearable="true" :placeholder="searchItem.name" @change="handleSearch">
                        <el-option v-for="(searchLabel, searchValue) in searchItem.value" :key="searchLabel+'+'+searchValue" :label="searchLabel" :value="searchValue"></el-option>
                    </el-select>
                </div>
            ...
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column prop="username" label="姓名" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-link type="primary" :underline="false" @click="handlePreview(scope)">{{ scope.row.username }}</el-link>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="source" label="操作来源"></el-table-column>
                <el-table-column prop="ctime" sortable="custom" label="拨号时间" width="140">
...
module.exports = {
    ...
    methods: {
        ...
        getBaseData() {
            let _this = this;
            httpPost('m=user&c=company_comlog&a=jobtellog_search_list', {}, {hideloading: true}).then(function (response) {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=company_comlog&a=jobtellog', params,{hideloading: true}).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=company_comlog&a=deljobtellog', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
