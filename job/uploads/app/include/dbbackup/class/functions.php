...
function BackupDatabaseInit($add){
    ...
    $data   =   array('a' => $phome, 't' => 0, 's' => 0, 'p' => 0, 'mypath' => $add['mypath'], 'waitbaktime' => $waitbaktime);
...
function BackupDatabaseFileSize($t, $s, $p, $mypath, $alltotal, $thenof, $fnum, $stime = 0)
{
    ...
    if ($t >= $count) {
        ...
        $errorMsg   =   "备份成功！整个过程耗时：".ToChangeUseTime($stime);
        $result =   array('error' => 2, 'msg' => $errorMsg);
        header('content-type:application/json; charset=utf-8');
        echo json_encode($result);
        exit;
    }
    ...
    foreach ($sql as $k => $r) {
        ...
        if (strlen($dumpsql) >= $b_filesize * 1024 || (($k + 1) >= $maxLimit && $s < $allnum)) {
            ...
            $data = array('a' => 'BackupDatabaseFileSize', 'phome' => 'BakExe', 't' => $t, 's' => $s, 'p' => $p, 'mypath' => $mypath, 'alltotal' => $num, 'thenof' => $noautof, 'fieldnum' => $fieldnum, 'stime' => $stime, 'waitbaktime' => $waitbaktime, 'collation' => $collation);
            $result =   array('error' => 0, 'msg' => '');
            ($data) && ($result['data'] = $data);
            header('content-type:application/json; charset=utf-8');
            echo json_encode($result);
    ...
    $data = array('a' => 'BackupDatabaseFileSize', 'phome' => 'BakExe', 't' => $t, 's' => 0, 'p' => 0, 'mypath' => $mypath, 'stime' => $stime, 'waitbaktime' => $waitbaktime);
    $result =   array('error' => 0, 'msg' => '');
    ($data) && ($result['data'] = $data);
    header('content-type:application/json; charset=utf-8');
    echo json_encode($result);
...
function RecoverDataReturnUrl($add,$mypath){
    global $db,$bakpath,$config;
    if(empty($mypath)||empty($add['mydbname'])){
