    ...
    <div class="moduleElHight">
                ...
                <div class="moduleInptList">
                    <el-input placeholder="输入你要搜索的关键字" @keyup.enter.native="handleSearch" size="small" v-model="searchForm.keyword" class="input-with-select" clearable>
                        <el-select v-model="searchForm.type" slot="prepend" placeholder="请选择">
                            ...
                            <el-option label="职位名称" value="3"></el-option>
                        </el-select>
                    </el-input>
                </div>
                <div class="tableSeachInpt tableSeachInptsmalltwo">
                    <el-date-picker v-model="searchForm.times" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="推送开始日期" end-placeholder="推送结束日期" :picker-options="timeOptions" value-format="yyyy-MM-dd" size="small" @change="handleTimeChange"></el-date-picker>
                </div>
        ...
        <div class="moduleElTable" :class="{ 'moduleElTableHig': tableHig }" style="border: 1px solid #ebeef5; width: calc(100% - 2px);">
            <el-table :data="tableData" style="width: 100%" stripe
                ...
                <el-table-column prop="username_n" label="姓名" width="150">
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.username_n }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="resume_name" label="被推送简历" min-width="150" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-button type="text" @click="handlePreview(scope)" style="padding: 0">{{ scope.row.resume_name }}</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="com_name" label="企业名称" min-width="200" show-overflow-tooltip>
                    <template slot-scope="scope">
                ...
                <el-table-column prop="ctime_n" label="推送时间" width="170"></el-table-column>
...
module.exports = {
    ...
    methods: {
        ...
        getList() {
            ...
            _this.loading = true;
            httpPost('m=user&c=users_userlog&a=trust', params,{hideloading: true}).then(function (response) {
        ...
        delete(params) {
            let _this = this;
            httpPost('m=user&c=users_userlog&a=deltrust', params).then(function (response) {
                let res = response.data;
                if (res.error === 0) {
